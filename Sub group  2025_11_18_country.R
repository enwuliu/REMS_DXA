...
create_meta_analysis <- function(place, bone_site, data = dat, output_path = "D:\\flinders\\REMS the accuracy of diagnostic tests for Osteoporosis\\") {
  
  # Filter data based on country and bone site
  filtered_data <- data %>% 
    filter(grepl(bone_site, site), grepl(place, Country))
  ...
  meta_result <- metacor(cor = filtered_data$r,
                         ...
                         prediction = TRUE,
                         title = paste(bone_site, "BMD -", place))
  
  # Create output filename
  filename <- paste0("forest_plot_", gsub(" ", "_", tolower(bone_site)), "_", 
                     gsub(" ", "_", tolower(place)), "_country.tiff")
...
italy_femoral <- create_meta_analysis("Italy", "Femoral")
summary(italy_femoral)

other_femoral <- create_meta_analysis("Other", "Femoral")
summary(other_femoral)
...
italy_lumbar <- create_meta_analysis("Italy", "Lumbar spine")
summary(italy_lumbar)

other_lumbar <- create_meta_analysis("Other", "Lumbar spine")
summary(other_lumbar)

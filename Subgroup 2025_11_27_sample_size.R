...
create_meta_analysis <- function(is_big, bone_site, data = dat, output_path = "D:\\flinders\\REMS the accuracy of diagnostic tests for Osteoporosis\\") {
  
  # Filter data based on country and bone site
  filtered_data <- data %>% 
    filter(grepl(bone_site, site), grepl(is_big, big_sample))
  ...
  meta_result <- metacor(cor = filtered_data$r,
                         ...
                         prediction=TRUE,
                         title = paste(bone_site, "BMD - is_big", is_big))
  
  # Create output filename
  filename <- paste0("forest_plot_", gsub(" ", "_", tolower(bone_site)), "_", 
                     gsub(" ", "_", tolower(is_big)), "_sample_size.tiff")
  full_path <- paste0(output_path, filename)
  
  # Generate forest plot
  tiff(full_path, width = 9, height = 8, units = "in", res = 600)
  meta::forest(
    meta_result,
    digits = 3,
    overall = TRUE,
